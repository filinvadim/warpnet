export default {
  ...
  WARPNET_TIMELINE(state, { tweets, nextToken }) {
    state.tweets.tweets = tweets;
    state.tweets.nextToken = nextToken;
  },
  WARPNET_CREATE(state, newTweet) {
    const tweets = [...state.tweets.tweets];
    state.tweets.tweets = tweets.unshift(newTweet);
  },

  WARPNET_FOLLOWERS(state, { profiles, nextToken }) {
    state.followers.followers = profiles;
    state.followers.nextToken = nextToken;
  },
  WARPNET_FOLLOWING(state, { profiles, nextToken }) {
    state.following.following = profiles;
    state.following.nextToken = nextToken;
  },

  WARPNET_LOADMORE_TWEETS(state, { tweets, nextToken }) {
    state.tweets.tweets = [...state.tweets.tweets, ...tweets];
    state.tweets.nextToken = nextToken;
  },

  WARPNET_SEARCH(state, { results, nextToken }) {
    state.search.results = results;
    state.search.nextToken = nextToken;
  },
  WARPNET_LOADMORE_SEARCH(state, { results, nextToken }) {
    state.search.results = [...state.search.results, ...results];
    state.search.nextToken = nextToken;
  },

  WARPNET_SEARCH_HASHTAG(state, { results, nextToken }) {
    state.search.results = results;
    state.search.nextToken = nextToken;
  },
  WARPNET_LOADMORE_SEARCH_HASHTAG(state, { results, nextToken }) {
    state.search.results = [...state.search.results, ...results];
    state.search.nextToken = nextToken;
  },

  WARPNET_NOTIFICATIONS_SUBSCRIBE(state, subscription) {
  ...
  WARPNET_NOTIFICATIONS_UNSUBSCRIBE(state) {
  ...
  WARPNET_NOTIFICATIONS_RESET(state) {
    state.notifications.newNotifications = 0;
  },
  WARPNET_CONVERSATIONS_LOAD(state, { conversations, nextToken }) {
    state.notifications.messages.conversations = conversations;
    state.notifications.messages.nextToken = nextToken;
  },
  WARPNET_MESSAGES_NEW(state, notification) {
  ...
  WARPNET_MESSAGES_RESET(state) {
    state.notifications.messages.newMessages = 0;
  },
  WARPNET_MESSAGES_LOAD(state, { messages, nextToken }) {
    state.notifications.messages.active.messages = messages;
    state.notifications.messages.active.nextTokenMessages = nextToken;
  },
  WARPNET_LOADMORE_MESSAGES(state, { messages, nextToken }) {
  ...
  WARPNET_MESSAGES_SEND(state, newMessage) {
    state.notifications.messages.active.messages = [
      ...state.notifications.messages.active.messages,
      newMessage,
    ];
  },
  WARPNET_CONVERSATION_ACTIVE_SET(state, conversation) {
    let set = state.notifications.messages.conversationsSet;
