...
ggen:
	...
	~/go/bin/oapi-codegen -import-mapping ./event.yml:github.com/filinvadim/warpnet/gen/event-gen -generate server,types,spec,skip-prune -package api ./spec/local-api.yml > server/api-gen/api.gen.go
tests:
	CGO_ENABLED=0 go test -count=1 -short ./...

prune:
	rm -rf $HOME/.badgerdb && rm -rf $HOME/.badgerdb
